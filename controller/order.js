...
const getAOrderDetail = async (req, res) => {
  ...
  if (!order)
    return res
      .status(400)
      .send(resWrapper("Order Not Found.", 404, null, "Id is not valid"));
...
const deleteAOrder = async (req, res) => {
  ...
  if (order.status !== "pending")
    return res
      ...
      .send(
        resWrapper(
          "Orders Can be Deleted Only when pending.",
          400,
          null,
          "Order can't be deletd After status is pending"
        )
      );
  ...
  return res
    .status(200)
    .send(resWrapper("Order Cancelled Successfully", 200, result));
};

const createAnOrder = async (req, res) => {
  const {
    error,
    value: { productsIds, shippingAddress },
    ...
    for (const item of productsIds) {
      ...
      if (!validator.isUUID(item.id)) {
        ...
        return res
          .status(400)
          .send(
            resWrapper("Invalid Uuid", 400, null, `Invalid Uuid ${item.id}`)
          );
      }
        ...
        return res
          ...
          .send(
            resWrapper(
              ...
              400,
              null,
              `Insufficient stock for product ${product.name} with Id: ${item.id}`
            )
          );
      }
  ...
  return res
    .status(201)
    .send(resWrapper("Order Placed Successfully", 201, temp));
};

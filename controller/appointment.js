...
const createAppointment = async (req, res) => {
  const {
    error,
    value: { appointmentDate, timeSlot, doctorId },
  ...
  if (!user)
    return res
      .status(404)
      .send(
        resWrapper("User Id is not Valid", 404, null, "User Id is not Valid")
      );
  if (!doctor)
    return res
      .status(404)
  ...
  if (!chkTimeSlotAvail)
    return res
      .status(400)
      .send(resWrapper("Invalid Time Slot", 400, null, "Invalid Time Slot"));
  ...
  return res
    .status(200)
    .send(resWrapper("Appointment Created Successfully", 200, temp));
};
...
const deleteAppointment = async (req, res) => {
  ...
  if (!appointment)
    return res
      .status(404)
      .send(
        resWrapper("Appointment not found", 404, null, "Appoint does not exist")
      );
  ...
  if (appointment.status !== "scheduled")
    return res
      ...
      .send(
        resWrapper(
          "Appointment can't be cancelled",
          400,
          null,
          "Appointment cancell error"
        )
      );
  ...
  return res
    .status(200)
    .send(resWrapper("Appointment Cancelled", 200, appointment));
};
...
const getAllAppointments = async (req, res) => {
  ...
  return res
    .status(200)
    .send(resWrapper("All Appointments received", 200, appointments));
};

...
const createDoctor = async (req, res) => {
  ...
  if (oldDoctor)
    return res
      ...
      .send(
        resWrapper(
          "Doctor With Email ALready Exist",
          400,
          null,
          "Email With Doctor Already Exist"
        )
      );
...
const getASingleDoctor = async (req, res) => {
  ...
  if (!doctor)
    return res
      .status(400)
      .send(resWrapper("Id is not Valid", 400, null, "Doctor not Found"));
...
const getTimeSlotOfADoctorByDate = async (req, res) => {
  ...
  const {
    error,
    value: { date },
  } = validateGetTimeSlotOfADoctorByDate({ date: paramsDate });
  if (error)
    return res
      .status(400)
      .send(resWrapper(error.message, 400, null, error.message));
  ...
  if (givenDate < todayDate)
    return res
      ...
      .send(
        resWrapper(
          "Date must be of today's are forward",
          400,
          null,
          "Date Error"
        )
  ...
  if (!chkDocAvail)
    return res.status(200).send(resWrapper("No Time Slot Available", 200, []));
  ...
  if (allAppointmentsOnThisDay.length === 0)
    return res
      .status(200)
      .send(
        resWrapper("Reterived All Time Slots", 200, doctor.availableTimeSlots)
      );

  const bookedTimeSlots = allAppointmentsOnThisDay.map(
    (appointment) => appointment.timeSlot
  );
  const remainingTimeSlots = doctor.availableTimeSlots.filter(
    (timeSlot) => !bookedTimeSlots.includes(timeSlot)
  );

  return res
    .status(200)
    .send(resWrapper("Reterived All Time Slots", 200, remainingTimeSlots));
};
